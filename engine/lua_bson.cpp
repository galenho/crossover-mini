...
int32_t get_length(const uint8_t * data) {
...
void bson_reserve(struct bson *b, int sz) {
	if (b->size + sz <= b->cap)
		return;
	do {
		b->cap *= 2;
...
void check_reader(lua_State *L, struct bson_reader *br, int sz) {
	if (br->size < sz) {
		luaL_error(L, "Invalid bson block (%d:%d)", br->size, sz);
	}
}

int read_byte(lua_State *L, struct bson_reader *br) {
	check_reader(L, br, 1);
...
int32_t read_int32(lua_State *L, struct bson_reader *br) {
	check_reader(L, br, 4);
	const uint8_t * b = br->ptr;
...
int64_t read_int64(lua_State *L, struct bson_reader *br) {
	check_reader(L, br, 8);
	const uint8_t * b = br->ptr;
...
lua_Number read_double(lua_State *L, struct bson_reader *br) {
	check_reader(L, br, 8);
	union {
		uint64_t i;
		double d;
...
const void* read_bytes(lua_State *L, struct bson_reader *br, int sz) {
	const void * r = br->ptr;
...
const char* read_cstring(lua_State *L, struct bson_reader *br, size_t *sz) {
	int i;
	for (i=0;;i++) {
		if (i==br->size) {
...
void write_byte(struct bson *b, uint8_t v) {
	bson_reserve(b,1);
	b->ptr[b->size++] = v;
}

void write_int32(struct bson *b, int32_t v) {
...
void write_length(struct bson *b, int32_t v, int off) {
	uint32_t uv = (uint32_t)v;
	b->ptr[off++] = uv & 0xff;
	b->ptr[off++] = (uv >> 8)&0xff;
...
void write_string(struct bson *b, const char *key, size_t sz) {
	bson_reserve(b,sz+1);
...
int reserve_length(struct bson *b) {
	int sz = b->size;
	bson_reserve(b,4);
	b->size +=4;
	return sz;
...
void write_int64(struct bson *b, int64_t v) {
...
void write_double(struct bson *b, lua_Number d) {
	union {
		double d;
		uint64_t i;
	} v;
...
void pack_dict(lua_State *L, struct bson *b, bool array, int depth);

void append_key(struct bson *bs, int type, const char *key, size_t sz) {
	write_byte(bs, type);
...
int is_32bit(int64_t v) {
	return v >= INT32_MIN && v <= INT32_MAX;
}

static void append_number(struct bson *bs, lua_State *L, const char *key, size_t sz) {
...
void append_table(struct bson *bs, lua_State *L, const char *key, size_t sz, int depth) {
...
void write_binary(struct bson *b, const void * buffer, size_t sz) {
...
void append_one(struct bson *bs, lua_State *L, const char *key, size_t sz, int depth) {
	int vt = lua_type(L,-1);
	switch(vt) {
	case LUA_TNUMBER:
...
int bson_numstr( char *str, unsigned int i ) {
	if ( i < MAX_NUMBER) {
...
void pack_dict_data(lua_State *L, struct bson *b, bool isarray, int depth, int kt) {
	char numberkey[32];
	const char * key = NULL;
	size_t sz;
	if (isarray) {
...
void pack_simple_dict(lua_State *L, struct bson *b, bool isarray, int depth) {
	if (depth > MAX_DEPTH) {
...
void pack_meta_dict(lua_State *L, struct bson *b, bool isarray, int depth) {
	if (depth > MAX_DEPTH) {
		luaL_error(L, "Too depth while encoding bson");
	}
	luaL_checkstack(L, 16, NULL);	// reserve enough stack space to pack table
...
void pack_ordered_dict(lua_State *L, struct bson *b, int n, int depth) {
	int length = reserve_length(b);
	int i;
	for (i=0;i<n;i+=2) {
		size_t sz;
...
int ltostring(lua_State *L) {
	size_t sz = lua_rawlen(L, 1);
	void * ud = lua_touserdata(L,1);
	lua_pushlstring(L, (char*)ud, sz);
	return 1;
}
...
void make_object(lua_State *L, int type, const void * ptr, size_t len) {
	luaL_Buffer b;
	luaL_buffinit(L, &b);
	luaL_addchar(&b, 0);
	luaL_addchar(&b, type);
	luaL_addlstring(&b, (char*)ptr, len);
	luaL_pushresult(&b);
}
...
void replace_object(lua_State *L, int type, struct bson * bs) {
	size_t len = 0;
...
int encode_bson(lua_State *L) {
	struct bson *b = (bson*)lua_touserdata(L, 2);
	lua_settop(L, 1);
...
int encode_bson_byorder(lua_State *L) {
	int n = lua_gettop(L);
	struct bson *b = (bson*)lua_touserdata(L, n);
	lua_settop(L, n-1);
...
int lsubtype(lua_State *L, int subtype, const uint8_t * buf, size_t sz) {
	switch(subtype) {
	case BSON_BINARY:
...
void typeclosure(lua_State *L) {
	...
	int i;
	int n = sizeof(type_name)/sizeof(type_name[0]);
	for (i=0;i<n;i++) {
		lua_pushstring(L, type_name[i]);
	}
