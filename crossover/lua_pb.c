...
static void lpb_encode(lpb_Env *e, const pb_Type *t) 
{
    ...
    while (lua_next(L, -2)) 
    {
        ...
        {
            const pb_Field *f = pb_fname(t, lpb_name(e->LS, lpb_toslice(L, -2)));
            if (f == NULL)
                /* skip */;
...
static int Lpb_encode_cmd(lua_State* L)
{
	int top = lua_gettop(L);
	assert(top == 3);
...
static int lpb_decode(lua_State *L, pb_Slice s, int start, int is_have_cmd) 
{
    int cmd = 0;
	if (is_have_cmd)
	{
		pb_readfixed32(&s, &cmd);
	}
...
static int Lpb_decode(lua_State *L) 
{
    return lpb_decode(L, lua_isnoneornil(L, 2) ?
            pb_lslice(NULL, 0) :
            lpb_checkslice(L, 2), 3, 0);
}

static int Lpb_decode_cmd(lua_State* L)
{
	return lpb_decode(L, lua_isnoneornil(L, 2) ?
		pb_lslice(NULL, 0) :
		lpb_checkslice(L, 2), 3, 1);
}
...
int luaopen_lua_pb(lua_State *L) {
    luaL_Reg libs[] = {
        { "pack",     Lbuf_pack     },
...
static int Lpb_decode_unsafe(lua_State *L) {
    ...
    return lpb_decode(L, pb_lslice(data, size), 4, 0);
}
